...
describe 'compiled component az-asg' do
  ...
  let(:template) { YAML.load_file("#{File.dirname(__FILE__)}/../out/tests/windows/az-asg.compiled.yaml") }
  ...
  context "Resource" do
    ...
    context "LaunchTemplate1" do
      ...
      it "to have property LaunchTemplateData" do
          expect(resource["Properties"]["LaunchTemplateData"]).to eq({"SecurityGroupIds"=>[{"Ref"=>"SecurityGroup"}], "TagSpecifications"=>[{"ResourceType"=>"instance", "Tags"=>[{"Key"=>"Name", "Value"=>{"Fn::Sub"=>"${EnvironmentName}-az-asg"}}, {"Key"=>"Environment", "Value"=>{"Ref"=>"EnvironmentName"}}, {"Key"=>"EnvironmentType", "Value"=>{"Ref"=>"EnvironmentType"}}, {"Key"=>"Role", "Value"=>{"Fn::Sub"=>"${RoleName}"}}, {"Key"=>"Name", "Value"=>{"Fn::Sub"=>["${EnvironmentName}-az-asg-${AZ}", {"AZ"=>{"Fn::Select"=>[1, {"Fn::GetAZs"=>{"Ref"=>"AWS::Region"}}]}}]}}]}, {"ResourceType"=>"volume", "Tags"=>[{"Key"=>"Name", "Value"=>{"Fn::Sub"=>"${EnvironmentName}-az-asg"}}, {"Key"=>"Environment", "Value"=>{"Ref"=>"EnvironmentName"}}, {"Key"=>"EnvironmentType", "Value"=>{"Ref"=>"EnvironmentType"}}, {"Key"=>"Role", "Value"=>{"Fn::Sub"=>"${RoleName}"}}, {"Key"=>"Name", "Value"=>{"Fn::Sub"=>["${EnvironmentName}-az-asg-${AZ}", {"AZ"=>{"Fn::Select"=>[1, {"Fn::GetAZs"=>{"Ref"=>"AWS::Region"}}]}}]}}]}], "UserData"=>{"Fn::Base64"=>{"Fn::Sub"=>["<powershell>\n$instanceId = invoke-restmethod -uri http://169.254.169.254/latest/meta-data/instance-id\ncfn-signal.exe -e $lastexitcode --region ${AWS::Region} --stack ${AWS::StackName} --resource AutoScaleGroup${AZId}\n</powershell>\n", {"AZId"=>1}]}}, "IamInstanceProfile"=>{"Name"=>{"Ref"=>"InstanceProfile"}}, "KeyName"=>{"Fn::If"=>["KeyPairSet", {"Ref"=>"KeyPair"}, {"Ref"=>"AWS::NoValue"}]}, "ImageId"=>{"Ref"=>"Ami"}, "InstanceType"=>{"Ref"=>"InstanceType"}, "InstanceMarketOptions"=>{"Fn::If"=>["SpotEnabled", {"MarketType"=>"spot", "SpotOptions"=>{"SpotInstanceType"=>"one-time"}}, {"Ref"=>"AWS::NoValue"}]}})
      end
    ...
    context "LaunchTemplate2" do
      ...
      it "to have property LaunchTemplateData" do
          expect(resource["Properties"]["LaunchTemplateData"]).to eq({"SecurityGroupIds"=>[{"Ref"=>"SecurityGroup"}], "TagSpecifications"=>[{"ResourceType"=>"instance", "Tags"=>[{"Key"=>"Name", "Value"=>{"Fn::Sub"=>"${EnvironmentName}-az-asg"}}, {"Key"=>"Environment", "Value"=>{"Ref"=>"EnvironmentName"}}, {"Key"=>"EnvironmentType", "Value"=>{"Ref"=>"EnvironmentType"}}, {"Key"=>"Role", "Value"=>{"Fn::Sub"=>"${RoleName}"}}, {"Key"=>"Name", "Value"=>{"Fn::Sub"=>["${EnvironmentName}-az-asg-${AZ}", {"AZ"=>{"Fn::Select"=>[2, {"Fn::GetAZs"=>{"Ref"=>"AWS::Region"}}]}}]}}]}, {"ResourceType"=>"volume", "Tags"=>[{"Key"=>"Name", "Value"=>{"Fn::Sub"=>"${EnvironmentName}-az-asg"}}, {"Key"=>"Environment", "Value"=>{"Ref"=>"EnvironmentName"}}, {"Key"=>"EnvironmentType", "Value"=>{"Ref"=>"EnvironmentType"}}, {"Key"=>"Role", "Value"=>{"Fn::Sub"=>"${RoleName}"}}, {"Key"=>"Name", "Value"=>{"Fn::Sub"=>["${EnvironmentName}-az-asg-${AZ}", {"AZ"=>{"Fn::Select"=>[2, {"Fn::GetAZs"=>{"Ref"=>"AWS::Region"}}]}}]}}]}], "UserData"=>{"Fn::Base64"=>{"Fn::Sub"=>["<powershell>\n$instanceId = invoke-restmethod -uri http://169.254.169.254/latest/meta-data/instance-id\ncfn-signal.exe -e $lastexitcode --region ${AWS::Region} --stack ${AWS::StackName} --resource AutoScaleGroup${AZId}\n</powershell>\n", {"AZId"=>2}]}}, "IamInstanceProfile"=>{"Name"=>{"Ref"=>"InstanceProfile"}}, "KeyName"=>{"Fn::If"=>["KeyPairSet", {"Ref"=>"KeyPair"}, {"Ref"=>"AWS::NoValue"}]}, "ImageId"=>{"Ref"=>"Ami"}, "InstanceType"=>{"Ref"=>"InstanceType"}, "InstanceMarketOptions"=>{"Fn::If"=>["SpotEnabled", {"MarketType"=>"spot", "SpotOptions"=>{"SpotInstanceType"=>"one-time"}}, {"Ref"=>"AWS::NoValue"}]}})
      end
